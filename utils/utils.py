...
def audios(flag,  dataset_name):
    ...
    for i in range(length):
        photo_path = all_label_json[i]['audio_path']
        # if dataset_name ==""
        # print(photo_path)
        photo_label = all_label_json[i]['photo_label']
        fprint_path = all_label_json[i]['fprint_path']
        dict={}
        dict['audio_path']=photo_path
        dict['photo_label'] =photo_label
        dict['fprint_path'] =fprint_path
...
def save_checkpoint_ljspeech(epoch,model, gpus, checkpoint_path, best_model_path, filename='_checkpoint.pth.tar'):
    epoch = epoch
    if(len(gpus) > 1):
        old_state_dict = model.state_dict()
        from collections import OrderedDict
    ...
    torch.save(state, filepath)
    # just save best model
    shutil.copy(filepath, best_model_path  + str(epoch) + '.pth.tar')
