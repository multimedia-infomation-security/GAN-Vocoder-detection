...
from model.lcnn import DGModel,Discriminator
...
from  loss.SuperLoss import SuperLoss,SuperLoss1,SuperLoss2
...
device = 'cuda'
# from experiment_fingerprint.reconstruct.model.VAE import ConvVAE
...
def train():
    ...
    tgt_valid_dataloader = get_dataset(config.src1_data,config.src2_data, config.src3_data,
                                       config.tgt_data, config.batch_size)
    ...
    DG_MODEL = DGModel(config.model)
    net = DG_MODEL.dg_model.to(device)
    ...
    start = timer()
    criterion = {
        'softmax': SuperLoss(C=2, lam=1).cuda(),
        'triplet': HardTripletLoss(margin=0.1, hardest=False).cuda(),
        'ad':SuperLoss2(C=2, lam=1)
    }
    optimizer_dict = [
    ...
    for iter_num in range(max_iter+1):
        ...
        input_fprint = torch.cat(
            [src1_fprint_real, src1_fprint_fake, src2_fprint_real, src2_fprint_fake, src3_fprint_real,
             src3_fprint_fake], dim=0)
        ...
        input_fprint= _compute_frontend(input_fprint)
        input_data= _compute_frontend(input_data)
        # recons_data = input_data - input_fprint
        recons_data= _compute_frontend(recons_data)
        input_fprint = net.feature(input_fprint)
        input = recons_data + input_fprint
        ######### forward #########
        classifier_label_out, feature= net(input)
        ...
        fake_shape_list = []
        fake_shape_list.append(input1_fake_shape)
        fake_shape_list.append(input2_fake_shape)
        fake_shape_list.append(input3_fake_shape)
        real_adloss = criterion['ad'](discriminator_out_real, real_shape_list)
        # fake_adloss = Fake_AdLoss(discriminator_out_fake, criterion["softmax"], fake_shape_list)
